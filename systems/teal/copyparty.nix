...
{ project, config, lib, ... }:
{
  ...
  config = {
    ...
    services.copyparty =
      ...
      in
      {
        ...
        settings = {
          ...
          # we have an IDP setup that is explained in full in "F01.23.01 Copyparty" -> allows tailscale-nginx-auth with fallback to oauth2-proxy
          idp-store = 3;

          # let users use their passwords for auth with clients that can't use oauth
          chpw = true;
          chpw-db = "/var/lib/copyparty/chpw.json";
        ...
        accounts = {
          coded.passwordFile = "/secrets/copyparty/default_password";
          minion.passwordFile = "/secrets/copyparty/default_password";
        };
    ...
    systemd.services.copyparty.serviceConfig.StateDirectory = "copyparty " + (lib.pipe config.services.copyparty.volumes [
      builtins.attrNames
      (map (mount: "copyparty/data${mount}"))
